...
export default function ProspectivePage() {
  ...
  async function openDoc(doc: PmsDocument) {
    ...
    if (doc.status === "SENT") {
      await supabase
        ...
        .eq("id", doc.id);
      setDocs((prev) => prev.map((d) => d.id === doc.id ? { ...d, status: "VIEWED" } : d));
    }
  }
  ...
  async function submitComment() {
    ...
    try {
      ...
      setDocs((prev) => prev.map((d) => d.id === selectedDoc.id ? { ...d, status: "COMMENT_REQUESTED", comment } : d));
      setSelectedDoc((prev) => prev ? { ...prev, status: "COMMENT_REQUESTED", comment } : prev);
  ...
  async function submitSignature() {
    ...
    try {
      ...
      await supabase
        ...
        .eq("id", selectedDoc.id);
      setDocs((prev) => prev.map((d) => d.id === selectedDoc.id ? { ...d, status: "SIGNED", signature_data: signatureData } : d));
      setSelectedDoc((prev) => prev ? { ...prev, status: "SIGNED" } : prev);
      setView("detail");
    ...
    <div className="max-w-md mx-auto bg-white min-h-screen flex flex-col font-sans">
      ...
      <main className="flex-1 p-5 space-y-4">
        ...
        <p className="text-[10px] text-slate-400 text-center">
          서명은 본인의 자필 서명과 동일한 법적 효력을 가질 수 있습니다.
        </p>
        <button
  ...
  if (view === "detail" && selectedDoc) {
    const typeInfo = TYPE_LABEL[selectedDoc.type];
    const statusInfo = STATUS_LABEL[selectedDoc.status];
      ...
      <div className="max-w-md mx-auto bg-white min-h-screen flex flex-col font-sans">
        <header className="bg-[#1a2a3a] text-white px-6 py-4">
          ...
          <div className="flex justify-between items-start">
            ...
            <span className={`text-[10px] font-bold px-2 py-1 rounded-full ml-2 flex-shrink-0 ${statusInfo.cls}`}>
              {statusInfo.label}
            </span>
          </div>
          <p className="text-xs text-slate-400 mt-1">
            {new Date(selectedDoc.sent_at).toLocaleDateString("ko-KR")} 수신
          </p>
        </header>
        <main className="flex-1 overflow-y-auto">
          ...
          {selectedDoc.html_content && (
            <div
              className="p-5 prose prose-sm max-w-none border-b border-slate-100"
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="border-t border-slate-100 pt-4">
                <button
                  ...
                  className="w-full bg-[#2a4d69] text-white font-bold py-3 rounded-xl"
                >
                  ✍️ 전자서명 진행
                </button>
                <p className="text-[10px] text-slate-400 text-center mt-2">
                  내용에 동의하시면 전자서명으로 의사를 표시해 주세요.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="p-5">
              <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-xl text-center">
                ...
                <p className="text-xs text-emerald-600 mt-1">
                  {selectedDoc.signed_at ? new Date(selectedDoc.signed_at).toLocaleString("ko-KR") : ""}
                </p>
              </div>
            </div>
          )}

          {/* Listing Report 하단 — rbs-homes 링크 */}
          {selectedDoc.type === "LISTING_REPORT" && (
            <div className="p-5 border-t border-slate-100">
              <div className="bg-gradient-to-r from-blue-700 to-indigo-800 text-white p-4 rounded-xl space-y-2">
                ...
                <a
                  href={process.env.NEXT_PUBLIC_RBS_HOMES_URL ?? "https://rbs-homes.com"}
                  target="_blank"
                  rel="noreferrer"
        ...
        <div>
          ...
          {docs.length === 0 ? (
            ...
            <div className="space-y-3">
              {docs.map((doc) => {
                const typeInfo = TYPE_LABEL[doc.type];
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className={`text-sm font-bold ${isUnread ? "text-blue-800" : "text-slate-800"}`}>
                            {doc.title}
                          </p>
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            {new Date(doc.sent_at).toLocaleDateString("ko-KR")}
                          </p>
                        </div>
                      </div>
